...
async function main() {
    ...
    const defaultIsResolved = 'false';

    try {
        const createdDate = new Date(2023, 0, 2, 11, 9, 20, 916);
        const mergeDate = new Date(2023, 0, 1, 11, 11, 20, 916);
        ...
        const startingIssueId = await question('Enter your starting github issue id eg. "1783828" ', `${defaultStartId + count}`);
        const startingIssueKey = await question('Enter your starting github issue key eg. "100" ', `${defaultStartKey + count}`);
        ...
        if (isResolved) {
            resolvedDate = await question('Enter the resolved date ', defaultResolvedDateString);
        }
        ...
        for(let i = 0; i < numberOfIssues; i++) {
            ...
            const issueValues = [
                ...
                isResolved ? '\'CLOSED\'' : '\'OPEN\'', isResolved ? `'${resolvedDate}'` : 'null', `'${createdDateString}'`, `'${createdDateString}'`, `''`, // 15-19
            ...
            const insertStatement = `INSERT INTO issues (${columnValues}) VALUES (${issueValues});\n\n`;
            const boardIssueStatement = `INSERT INTO board_issues (${boardIssueColumnValues}) VALUES (${boardIssueValues});\n\n`;
